...
#         print(im.shape)
        y_min=0
        x_min=0
        subarr = np.zeros((self.height, self.width, 3))
        subarr[y_min:y_min + src_im_height, x_min:x_min + src_im_width,:] = im[y_min:y_min + src_im_height,
                            x_min:x_min + src_im_width,
                            :]
        if self.aug is not None:
            subarr = self.aug(image=subarr)['image']
        output_arr.append(subarr)
