...
from .hrnet_config import parse
import yaml
config_path = '../nets/zoo/hrnet.yml'
...
class hrnetv2_BE(nn.Module):

    def __init__(self,norm_layer=None, pretrained=False , mode='Train'):
        super(hrnetv2_BE, self).__init__()
        self.mode=mode
        if norm_layer is None:
            norm_layer = nn.BatchNorm2d
        self.norm_layer = norm_layer
        ...
        self.fuse = nn.Sequential(nn.Conv2d(4, 32, 1),nn.ReLU(inplace=True))
...
#         print(len(y_list))
        ...
        d1 = self.dsn1(s1)
        d2 = F.upsample_bilinear(self.dsn2(s2), size=(h,w))
        d3 = F.upsample_bilinear(self.dsn3(s3), size=(h,w))
        d4 = F.upsample_bilinear(self.dsn4(s4), size=(h,w))
