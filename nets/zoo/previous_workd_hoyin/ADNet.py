...
class XDXD_SpaceNet4_UNetVGG16(nn.Module):
    ...
    def __init__(self, num_filters=32, pretrained=False, channels=3, num_of_layers=15):
        super().__init__()
        kernel_size = 3
        padding = 1
        features = 128
        ...
        self.conv3 = nn.Conv2d(in_channels=6,out_channels=2,kernel_size=1,stride=1,padding=0,groups=1,bias=True)
        self.ReLU = nn.ReLU(inplace=True)
        self.Tanh= nn.Tanh()
