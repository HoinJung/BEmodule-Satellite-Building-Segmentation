...
class XDXD_SpaceNet4_UNetVGG16(nn.Module):
    def __init__(self, num_filters=32, pretrained=False):
        ...
        self.DCR_block51 = self.make_layer(_DCR_block, 512)
        self.DCR_block52 = self.make_layer(_DCR_block, 512)
        self.ASPP=self.make_layer(ASPP, 1024)
        ...
        self.up_deconv3=self.make_layer(_up_deconv,[512,128])
        self.DCR_block33 = self.make_layer(_DCR_block, 384)
        self.DCR_block34 = self.make_layer(_DCR_block, 384)
        self.up2 = self.make_layer(_up, 384)
        self.up_deconv2=self.make_layer(_up_deconv,[384,96])
        self.DCR_block23 = self.make_layer(_DCR_block, 256)
        self.DCR_block24 = self.make_layer(_DCR_block, 256)
        self.up1 = self.make_layer(_up, 256)
        self.up_deconv1=self.make_layer(_up_deconv,[256,64])
        self.DCR_block13 = self.make_layer(_DCR_block, 160)
        self.DCR_block14 = self.make_layer(_DCR_block, 160)
        self.conv_f1 = nn.Conv2d(in_channels=160, out_channels=32, kernel_size=1, stride=1, padding=0)
