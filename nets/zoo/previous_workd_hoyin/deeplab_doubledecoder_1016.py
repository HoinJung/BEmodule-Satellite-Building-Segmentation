...
class _up_deconv(nn.Module):
    def __init__(self, in_channels, out_channels):
        ...
        self.bn_i = nn.BatchNorm2d(out_channels)
        self.relu = nn.PReLU()
...
class ConvRelu(nn.Module):
    def __init__(self, in_, out):
        ...
        #self.bn = nn.BatchNorm2D(out) #1019
...
class XDXD_SpaceNet4_UNetVGG16(nn.Module):
    def __init__(self, num_filters=16, pretrained=False):
        ...
        self.bi_up3 = _up_deconv(num_filters * 64, num_filters * 16)
        self.bi_up2 = _up_deconv(num_filters * 32, num_filters * 8)
        self.bi_up1 = _up_deconv(num_filters * 16, num_filters * 4)
        ...
        self.conv_f1 = nn.Conv2d(in_channels=num_filters*16, out_channels=num_filters*8, kernel_size=3, stride=1, padding=1)
        self.conv_f2 = nn.Conv2d(in_channels=num_filters*8, out_channels=self.num_classes, kernel_size=1, stride=1, padding=0)
        self.relu_f = nn.PReLU()
    ...
    def forward(self, x):
        ...
        out = self.conv_f1(concat_f)
        out =  self.relu_f(out)
        result = self.conv_f2(out)
