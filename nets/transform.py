...
from albumentations.augmentations.crops import Crop,CenterCrop,RandomCrop,  RandomSizedCrop
...
from albumentations.augmentations.transforms import VerticalFlip, ChannelShuffle, RandomShadow, RandomBrightnessContrast,      \
    HorizontalFlip, Flip, Transpose,  Cutout,  \
...
__all__ = ['Crop', 'VerticalFlip', 'HorizontalFlip', 'Flip', 'Transpose','ChannelShuffle','RandomShadow', 'ShiftScaleRotate',  'RandomBrightnessContrast',
...
def process_aug_dict(pipeline_dict, meta_augs_list=['oneof', 'oneorother','two']):
...
def get_augs(aug_dict, meta_augs_list=['oneof', 'oneorother','two']):
...
aug_matcher = {
    'crop': Crop, 'centercrop': CenterCrop, 'randomcrop': RandomCrop,'channelshuffle' : ChannelShuffle, 'randomshadow' : RandomShadow,  
