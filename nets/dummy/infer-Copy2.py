...
#                                             method=self.stitching_method)
            stitched_result_prev = np.transpose(subarr_preds, (0, 2, 3, 1))
            stitched_result = np.squeeze(stitched_result_prev[0], axis=2)
            print(stitched_result.shape)
            temp_arr= Image.fromarray(stitched_result).convert('L')
            temp_arr2= Image.fromarray(stitched_result[:650,:650])
            temp_arr.save(str(idx) + "_1_temp.tif")
            temp_arr2.save(str(idx) + "_2_temp.tif")
