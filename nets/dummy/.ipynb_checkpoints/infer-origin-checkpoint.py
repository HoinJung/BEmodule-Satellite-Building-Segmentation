...
#                 subarr_preds = self.model(inf_input)
#                 # bear's add part : activation elementwise - below one line
#                 subarr_preds = F.softmax(subarr_preds, dim=1)
#                 #subarr_preds = F.sigmoid(subarr_preds)
#                 subarr_preds = subarr_preds.cpu().data.numpy()
...
#             # bear's add part
#             sp_str = os.path.split(im_path)[1].split(".")
#             building = sp_str[0] + "background.." + sp_str[1]
#             stitched_result_background = stitched_result[0,:,:]
#             stitched_result_building = stitched_result[1,:,:]
#             stitched_result = stitched_result[1,:,:]
            
#             stitched_result = stitched_result[1,:,:] - stitched_result[0,:,:]
